...
def alexnet_v2_captcha_multi(inputs,
               ...
               dropout_keep_prob=0.5,
               spatial_squeeze=True,
               scope='alexnet_v2_captcha_multi',
               global_pool=False):
  # 为了识别4位验证码而改装的多任务联合训练
  with tf.variable_scope(scope, 'alexnet_v2', [inputs]) as sc:
    end_points_collection = sc.original_name_scope + '_end_points'
    # Collect outputs for conv2d, fully_connected and max_pool2d.
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
...
def alexnet_v2_captcha_single(inputs,
                      num_classes=1000,
                      is_training=True,
                      dropout_keep_prob=0.5,
                      spatial_squeeze=True,
                      scope='alexnet_v2_captcha_single',
                      global_pool=False):
  # 为了识别4位验证码而改装的单任务训练
  with tf.variable_scope(scope, 'alexnet_v2', [inputs]) as sc:
    end_points_collection = sc.original_name_scope + '_end_points'
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      with slim.arg_scope([slim.conv2d],
        ...
        if num_classes:
          ...
          net = slim.conv2d(net, num_classes*4, [1, 1],
                            activation_fn=None,
                            normalizer_fn=None,
                            biases_initializer=tf.zeros_initializer(),
                            scope='fc8_0')
          ...
          if spatial_squeeze:
            # 删除多余的维度 或者说挤压掉长度为1的维度
            net = tf.squeeze(net, [1, 2], name='fc8_0/squeezed')
            end_points[sc.name + '/fc8_0'] = net
      return net, end_points
